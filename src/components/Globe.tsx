...
const PINS = [
  { id: 'gs',      lat: 40.7128, lng: -74.006,  city: 'New York, NY',   company: 'Goldman Sachs',     role: 'Senior Associate, Global Banking and Markets Transformation', stat: '$580M entity migration. 60% review-cycle reduction. CFTC framework in 30 days.', period: '2023 to Present' },
  { id: 'carlyle', lat: 38.9072, lng: -77.0369, city: 'Washington, DC', company: 'The Carlyle Group', role: 'Associate, EMEA Fund Operations',                              stat: 'Salesforce CRM built from scratch. Investor onboarding adopted as global standard.', period: '2021 to 2023' },
  { id: 'trowe',   lat: 39.2904, lng: -76.6122, city: 'Baltimore, MD',  company: 'T. Rowe Price',     role: 'Analyst, Central Operations',                                  stat: 'Foundation in trade settlement, position reconciliation, and global investment operations.', period: '2019 to 2021' },
]
...
export default function Globe() {
  ...
  useEffect(() => {
    ...
    const rim = new THREE.PointLight(0xd97757, 0.55); rim.position.set(-8, -6, -8); scene.add(rim)

    const g = new THREE.Group(); scene.add(g)
    g.add(new THREE.Mesh(new THREE.SphereGeometry(R, 64, 64), new THREE.MeshStandardMaterial({ color: 0xeae3da, roughness: 0.85, metalness: 0.05 })))
    g.add(new THREE.Mesh(new THREE.SphereGeometry(R + 0.07, 32, 32), new THREE.MeshStandardMaterial({ color: 0xd97757, transparent: true, opacity: 0.05, side: THREE.BackSide })))

    const gm = new THREE.LineBasicMaterial({ color: 0xd97757, transparent: true, opacity: 0.18 })
    ...
    arc(38.9072, -77.0369, 51.5074, -0.1278, 1.45, 0xd97757, 0.7)
    arc(40.7128, -74.006, 38.9072, -77.0369, 1.12, 0x111111, 0.2)

    const pins: { mesh: THREE.Mesh; halo: THREE.Mesh; ring: THREE.Mesh }[] = []
    PINS.forEach(p => {
      const pos = ll(p.lat, p.lng, R + 0.04)
      const mesh = new THREE.Mesh(new THREE.SphereGeometry(0.065, 16, 16), new THREE.MeshStandardMaterial({ color: 0xd97757, emissive: 0xd97757, emissiveIntensity: 0.9 }))
      mesh.position.copy(pos); mesh.userData.id = p.id; g.add(mesh)
      const halo = new THREE.Mesh(new THREE.RingGeometry(0.1, 0.15, 32), new THREE.MeshBasicMaterial({ color: 0xd97757, transparent: true, opacity: 0.32, side: THREE.DoubleSide }))
      halo.position.copy(pos); halo.lookAt(new THREE.Vector3(0, 0, 0)); g.add(halo)
      const ring = new THREE.Mesh(new THREE.RingGeometry(0.16, 0.2, 32), new THREE.MeshBasicMaterial({ color: 0xd97757, transparent: true, opacity: 0.12, side: THREE.DoubleSide }))
  ...
  return (
    <section style={{ borderTop: LINE, padding: '5rem 2.5rem 6rem', background: C.bg }}>
      <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}
        ...
        <div>
          <p style={{ fontFamily: F.sans, fontSize: 10, letterSpacing: '0.15em', textTransform: 'uppercase', color: C.muted, marginBottom: 10 }}>Global footprint</p>
          <h2 style={{ fontFamily: F.serif, fontSize: 'clamp(1.8rem,3.5vw,3rem)', fontWeight: 300, letterSpacing: '-0.025em', color: C.text, lineHeight: 1.1 }}>
            East Coast foundation. <em style={{ color: C.accent, fontStyle: 'italic' }}>EMEA reach.</em>
          </h2>
        </div>
        <p style={{ fontFamily: F.sans, fontSize: 12, fontWeight: 300, color: C.muted, textAlign: 'right', lineHeight: 1.6, maxWidth: '22ch' }}>Drag to spin. Click a pin to explore each role.</p>
      </motion.div>
        ...
        <AnimatePresence>
          {loc && (
            <motion.div key={loc.id} initial={{ opacity: 0, y: 12, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.97 }} transition={{ duration: 0.28, ease: [0.22, 1, 0.36, 1] }}
              style={{ position: 'absolute', bottom: '1.5rem', left: '50%', transform: 'translateX(-50%)', background: 'rgba(250,248,245,0.98)', border: `1px solid ${C.accent}`, padding: '1.5rem 2rem', maxWidth: 420, width: '90%', zIndex: 10, backdropFilter: 'blur(16px)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                <div>
                  <p style={{ fontFamily: F.sans, fontSize: 9.5, letterSpacing: '0.14em', textTransform: 'uppercase', color: C.accent, marginBottom: 4 }}>{loc.period} · {loc.city}</p>
                  <h4 style={{ fontFamily: F.serif, fontSize: '1.5rem', fontWeight: 400, color: C.text, letterSpacing: '-0.02em' }}>{loc.company}</h4>
                </div>
                <button onClick={() => setActive(null)} style={{ background: 'none', border: 'none', color: C.muted, fontSize: 22, lineHeight: 1, cursor: 'pointer', padding: '0 4px' }}>×</button>
              </div>
              <p style={{ fontFamily: F.sans, fontSize: 12, fontWeight: 300, color: C.muted, marginBottom: 10, lineHeight: 1.5 }}>{loc.role}</p>
              <div style={{ borderTop: '1px solid rgba(217,119,87,0.2)', paddingTop: 10 }}>
                <p style={{ fontFamily: F.sans, fontSize: 13, color: C.accent, lineHeight: 1.6 }}>{loc.stat}</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center', marginTop: '2rem', flexWrap: 'wrap' }}>
        {PINS.map(p => (
          <button key={p.id} onClick={() => setActive(active === p.id ? null : p.id)} style={{ fontFamily: F.sans, fontSize: 10.5, letterSpacing: '0.08em', textTransform: 'uppercase', color: active === p.id ? C.accent : C.muted, border: `1px solid ${active === p.id ? 'rgba(217,119,87,0.5)' : C.line}`, background: active === p.id ? 'rgba(217,119,87,0.05)' : 'transparent', padding: '7px 16px', cursor: 'pointer', transition: 'all 0.2s' }}>
            {p.city}
          </button>
        ))}
      </div>
